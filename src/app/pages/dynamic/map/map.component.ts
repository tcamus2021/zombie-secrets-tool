...
import { EndingComponent } from '../../../common/map/ending/ending.component';
import { EmptyStepComponent } from '../../../common/map/empty-step/empty-step.component';
		...
		if (changes['map'] && changes['map'].currentValue) {
			...
			this.cmsClient.getZombieMap(mapId).subscribe({
				next: (response): void => {
					...
					this.stepState = Number.parseInt(
						localStorage.getItem(this.mapDetail.name) || '0'
					);
					if (isLastStep(this.stepState, this.mapDetail.steps)) {
						this.finished = true;
						this.actualStep =
							this.mapDetail.steps[
								this.mapDetail.steps.length - 1
							];
					} else {
						this.actualStep =
							this.mapDetail.steps[this.stepState] || undefined;
					}
					this.skippedSteps = this.mapDetail.steps.slice(
						0,
						this.stepState
					);
				},
